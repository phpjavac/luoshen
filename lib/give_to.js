    ...
    request(
      ...
      (err, response, body) => {
        ...
        if (!type) {
          ...
          // console.log(1)
          resolve(`分配${trueName}的任务一${taskName.innerHTML}失败。-${process.env.JIRA_URL}browse/${task}`);
          return;
        }
        ...
        request(
          ...
          (err, response, body) => {
            if (body.includes("<h1>会话过期</h1>")) {
              console.error(`分配${trueName}的任务一${taskName.innerHTML}失败。- jira状态失效`);
              resolve(`分配${trueName}的任务一${taskName.innerHTML}失败。- jira状态失效`);
              return;
            }
            if (err) {
              console.error(`分配${trueName}的任务一${taskName.innerHTML}失败。`);
            ...
            resolve(`分配${trueName}的任务一${taskName.innerHTML}成功。-${process.env.JIRA_URL}browse/${task}`);
          }
        );
      }
    );
